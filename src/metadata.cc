...
FLAC__Metadata_SimpleIterator* __FLAC__metadata_simple_iterator_new_async(BindingWorker<FLAC__Metadata_SimpleIterator*>* worker) {
  return FLAC__metadata_simple_iterator_new();
}

NAN_METHOD(__FLAC__metadata_simple_iterator_new) {
  Nan::Callback* cb = new Nan::Callback(Nan::To<Function>(info[info.Length() - 1]).ToLocalChecked());
  Nan::AsyncQueueWorker(new BindingWorker<FLAC__Metadata_SimpleIterator*>(cb,
